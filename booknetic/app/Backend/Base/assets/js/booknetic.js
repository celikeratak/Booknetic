...
var booknetic =
{
	...
	$element.daterangepicker(
		{
			...
			ranges: {
				Today: [moment(), moment()],
				[booknetic.__("This Week")]: [moment().startOf("week"), moment().endOf("week")],
				[booknetic.__("Last Week")]: [
					moment().subtract(1, "week").startOf("week"),
					moment().subtract(1, "week").endOf("week"),
				],
				[booknetic.__("This Month")]: [moment().startOf("month"), moment().endOf("month")],
				[booknetic.__("Last Month")]: [
					moment().subtract(1, "month").startOf("month"),
					moment().subtract(1, "month").endOf("month"),
				],
				[booknetic.__("This Year")]: [moment().startOf("year"), moment().endOf("year")],
	...
	$(tableSelector).find(".dt-paging-button.next").toggleClass("disabled", perPage < 10 || isLastPage);
	$(tableSelector).find(".dt-paging-button.previous").toggleClass("disabled", isFirstPage);

	if (isBetweenPages) {
		$(tableSelector).find(".dt-paging-button.next").removeClass("disabled");
		$(tableSelector).find(".dt-paging-button.previous").removeClass("disabled");
	}
	...
	const formattedPageTemplate = pageTemplate.replace(/__page__|__pages__/g, match => match === '__page__' ? currentPage : totalPages);
