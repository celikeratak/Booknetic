...
class Controller extends \BookneticApp\Providers\Core\Controller
{
    public function index()
    {
        ...
        $appointments = Appointment::leftJoin( 'customer', [ 'first_name', 'last_name', 'email', 'profile_image', 'phone_number' ] )
            ...
            ->selectSubQuery( $totalPrice, 'total_amount' );
        ...
        $dataTable->addColumns( bkntc__( 'STATUS' ), function ( $appointment ) {
            ...
            if ( $appointment[ 'payment_status' ] == 'pending' ) {
                $statusBtn = '<button type="button" class="btn btn-xs btn-light-warning cursor-default">' . bkntc__( 'Pending' ) . '</button>';
            } else if ( $appointment[ 'payment_status' ] == 'paid' ) {
                if ( $paidAmount < $totalAmount ) {
                    $statusBtn = '<button type="button" class="btn btn-xs btn-light-primary cursor-default">' . bkntc__( 'Paid (deposit)' ) . '</button>';
                } else {
                    $statusBtn = '<button type="button" class="btn btn-xs btn-light-success cursor-default">' . bkntc__( 'Paid' ) . '</button>';
                }
            } else if ( $appointment[ 'payment_status' ] == 'canceled' ) {
                $statusBtn = '<button type="button" class="btn btn-xs btn-light-danger cursor-default">' . bkntc__( 'Canceled' ) . '</button>';
            } else if ( $appointment[ 'payment_status' ] == 'not_paid' ) {
                $statusBtn = '<button type="button" class="btn btn-xs btn-light-default text-nowrap cursor-default">' . bkntc__( 'Not paid' ) . '</button>';
            }
