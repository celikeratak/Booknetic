...
class Ajax extends \BookneticApp\Providers\Core\Controller
{
	...
	public function save_payment()
	{
		...
		if( $appointmentId <= 0 || ! is_array( $prices ) || empty( $prices ) || is_null( $paid_amount ) || is_null( $status ) )
		{
			return $this->response( false );
		}

		$info = AppointmentSmartObject::load( $appointmentId );
		...
		return $this->response(true, [ 'id' => $appointmentId ]);
	}
