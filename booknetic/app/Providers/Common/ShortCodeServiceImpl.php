...
class ShortCodeServiceImpl
{
    ...
    public static function replace($text, $data)
    {
        if( ! empty( $data['appointment_id'] ) )
        {
            ...
            {
                $serviceExtraList[] = $extraInf->extra_name . ( $extraInf->quantity > 1 ? ' x' . $extraInf->quantity : '' ) . ' - ' . Helper::price( $extraInf->price * $extraInf->quantity );
            }
        ...
        {
            $locationInf = Location::withTranslations()->get( $data[ 'location_id' ] );
