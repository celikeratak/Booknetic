...
class Controller extends \BookneticApp\Providers\Core\Controller
{
	public function index()
	{
		...
		$dataTable->addColumns(bkntcsaas__('STATUS'), function( $tenant )
		{
            ...
            else if( ! empty( $tenant['active_subscription'] ) )
            {
                $statusBtn = '<button type="button" class="btn btn-xs btn-light-warning text-nowrap">'.bkntcsaas__('Expired').'</button>';
            }
            else if( empty( $tenant['verified_at'] ) )
            {
                $statusBtn = '<button type="button" class="btn btn-xs btn-light-warning text-nowrap">'.bkntcsaas__('Not activated').'</button><br/><button type="button" class="btn btn-xs btn-default mt-1 resend_activation_email_btn">'.bkntcsaas__('Resend activation email').'</button>';
            }
			else
			{
