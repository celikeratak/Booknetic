...
class CustomerPanelHelper
{
	...
	public static function canRescheduleAppointment( $appointment )
	{
		...
		if ( Helper::getOption('customer_panel_allow_   reschedule', 'on') != 'on' )
			return false;
	...
	public static function getLimitedBookingDays( int $serviceId )
	{
		$defaultOption = Helper::getOption( 'available_days_for_booking', '365' );
		$serviceOptionEnabled = Service::getData( $serviceId, 'enable_limited_booking_days', 0 );

		if ( empty( $serviceOptionEnabled ) ) {
			return $defaultOption;
		}

		return Service::getData( $serviceId, 'available_days_for_booking', $defaultOption );
	}
}